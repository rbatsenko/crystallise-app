...
const stickyNotes = [
  ...
  // Bottom corners — well away from logo
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <section ref={heroRef} className="relative h-screen overflow-hidden bg-[#fdfbf7]">
        ...
        >
          {navItems.map((item, i) => (
            <motion.div
              ...
              style={{ transformStyle: "preserve-3d", marginLeft: `${item.offsetX}px` }}
            >
              <Link
                href={item.href}
                className="block px-8 py-3 text-center font-mono font-bold text-2xl tracking-[0.15em] text-[#f4f4f4] uppercase bg-[#1a1a1a]"
                style={{
        ...
        {/* Sticky notes — each is its own isolated component, no shared state */}
        {stickyNotes.map((note, i) => (
          <StickyNote key={i} index={i} constraintsRef={heroRef} {...note} />
        ))}

        {/* Polaroid cards */}
        {polaroids.map((p, i) => (
          <PolaroidCard key={`polaroid-${i}`} index={i} constraintsRef={heroRef} {...p} />
        ))}
